...
export default async (sock, msg, args) => {
    ...
    const charMaps = {
        ...
        sansBold: "𝗔𝗕𝗖𝗗𝗘𝗙𝗚𝗛𝗜𝗝𝗞𝗟𝗠𝗡𝗢𝗣𝗤𝗥𝗦𝗧𝗨𝑽𝗪𝗫𝗬𝗭𝗮𝗯𝗰𝗱𝒆𝗳𝒈𝗵𝗶𝗷𝒌𝒍𝗺𝗻𝗼𝒑𝗾𝗿𝘀𝒕𝘂𝒗𝘄𝘅𝘆𝘇",
        sansItalic: "𝘈𝘉𝘊𝘋𝘌𝘍𝘎𝘏𝘐𝘑𝘒𝘓𝘔𝘕𝘖𝘗𝘘𝘙𝘚𝘛𝘜𝘝𝘞𝘟𝘠𝘡𝘢𝘣𝘤𝘥𝘦𝘧𝘨𝘩𝘪𝘫𝘬𝘭𝘮𝘯𝘰𝘱𝘲𝘳𝘴𝘵𝘶𝒗𝘸𝘹𝘺𝘻",
    ...
    const decorate = (input, symbol) => input.toUpperCase().split('').map(c => c + symbol).join('');

    let result = "";
    let i = 1;

    // 1. സാധാരണ മാപ്പിംഗ് ഫോണ്ടുകൾ
    for (const key in charMaps) {
        const charMap = Array.from(charMaps[key]);
        const styled = text.split('').map(char => {
            const index = normalChars.indexOf(char);
            return index !== -1 ? charMap[index] : char;
        }).join('');
        result += `*${i++}.* \`${styled}\`\n`;
    }

    // 2. സ്പെഷ്യൽ ചിഹ്നങ്ങൾ ചേർത്ത ഫോണ്ടുകൾ (Tap to copy enabled)
    const upperText = text.toUpperCase();
    result += `*${i++}.* \`${decorate(upperText, "͓̽")}\`\n`; // X's
    result += `*${i++}.* \`${decorate(upperText, "͛")}\`\n`; // Lightning
    result += `*${i++}.* \`${decorate(upperText, "͛⦚")}\`\n`; // Zigzags
    result += `*${i++}.* \`${decorate(upperText, "͎")}\`\n`; // UpArrows
    result += `*${i++}.* \`${decorate(upperText, "̺͆")}\`\n`; // VerticalBrackets
    result += `*${i++}.* \`${decorate(upperText, "≋")}\`\n`; // Tildes
    result += `*${i++}.* \`${upperText.split('').map(c => `【${c}】`).join('')}\`\n`; // Brackets
    result += `*${i++}.* \`${upperText.split('').map(c => ` ░${c}`).join('')}░\`\n`; // Shade
